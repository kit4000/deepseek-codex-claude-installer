...
現行確認済みは Claude `2.7032.0` / patch `2026-09-23.1` です。Opus 5.5 と Fable 5.1 は公式モデルのまま一覧に残し、DeepSeek は借り枠だけです。公式アプリが DeepSeek だけになるときは、`--apply` が `Claude-3p` の専用設定を公式アカウントへ戻します。
