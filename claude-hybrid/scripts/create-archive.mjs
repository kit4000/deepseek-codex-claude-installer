...
const outputPath = resolve(process.argv[2] ?? `/tmp/claude-hybrid-${timestamp}.tar.gz`);
...
if (relativeOutput === "" || (!relativeOutput.startsWith("..") && !relativeOutput.startsWith("/"))) {
  throw new Error("Write the archive outside the source bundle");
}
...
const manifestPath = resolve(projectRoot, "config/hybrid-files.json");
const files = JSON.parse(await readFile(manifestPath, "utf8"));
if (!Array.isArray(files) || files.length === 0) {
  throw new Error("hybrid-files.json must contain a non-empty file list");
}
...
for (const file of files) {
  ...
  if (!metadata.isFile()) throw new Error(`Archive entry is not a regular file: ${file}`);
  const contents = await readFile(path, "utf8");
  if (probableSecret.test(contents)) throw new Error(`Probable API credential found in archive file: ${file}`);
}
